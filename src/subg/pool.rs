...
pub fn load_pool(pool_path: &str) -> SubnetPool {
    ...
    fn from_reader<'a, E: Error>(
        reader: &'a File,
        from_reader: fn(&'a File) -> Result<SubnetPool, E>,
    ) -> SubnetPool {
        result(
            from_reader(reader),
            exitcode::DATAERR,
...
pub fn store_pool(pool_path: &str, pool: &SubnetPool) {
    let (path, pool_format) = parse_pool_path(pool_path);
    ...
    fn to_writer<'a, E: Error>(
        writer: &'a File,
        to_writer: fn(&'a File, &SubnetPool) -> Result<(), E>,
