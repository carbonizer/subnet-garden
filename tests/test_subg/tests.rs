...
use subnet_garden::subg;
use subnet_garden::subg::args::DEFAULT_STORAGE_PATH;
...
impl Test {
    pub(crate) fn store(&self) {
        subg::pool::store_pool(self.pool_path.to_str().unwrap(), &self.pool);
    }

    pub(crate) fn load(&mut self) {
        self.pool = subg::pool::load_pool(self.pool_path.to_str().unwrap());
    }
}

pub(crate) fn new_test_with_path(path: &str) -> Test {
    let mut test = assert_cmd::Command::cargo_bin(env!("CARGO_BIN_EXE_subg")).unwrap();
